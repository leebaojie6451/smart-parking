...
public interface CfPaymentAgencySwagger {
    ...
    @ApiOperation(value = "根据条件查询支付机构数据列表")
    @ApiImplicitParams({
    ...
    @ApiOperation(value = "根据query体查询数据")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "authorization", value = "jwt串(请加\"Bearer \"前缀，注意有空格)", required = true, paramType = "header", dataType = "string"),
            @ApiImplicitParam(name = "lang", value = "语言(zh-中文[默认]/en-英文...更多见官网文档)", required = false, paramType = "header", dataType = "string")
    })
    ...
    public ResponseResult add(CfPaymentAgencyForm cfPaymentAgencyForm);
    ...
    public ResponseResult update(CfPaymentAgencyForm cfPaymentAgencyForm);
